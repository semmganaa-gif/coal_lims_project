...
Coal LIMS системийн өдөр тутмын автомат backup хийх заавар.
...
Task Scheduler дээр:
1. `Coal LIMS Daily Backup` task дээр right-click
2. **Run** сонгох
...
Register-ScheduledTask -TaskName "Coal LIMS Daily Backup" -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description "PostgreSQL database backup every day at 2:00 AM"
```
...
├── coal_lims_20251218_020000.sql      # PostgreSQL dump
├── coal_lims_20251217_020000.sql
├── coal_lims_20251216_020000.sql
...
1. Task Scheduler → Coal LIMS Daily Backup
...
if %ERRORLEVEL% EQU 0 (
    powershell -Command "Send-MailMessage -To 'admin@example.com' -From 'backup@example.com' -Subject 'Coal LIMS Backup Success' -Body 'Daily backup completed successfully.' -SmtpServer 'smtp.example.com'"
) else (
    powershell -Command "Send-MailMessage -To 'admin@example.com' -From 'backup@example.com' -Subject 'Coal LIMS Backup FAILED' -Body 'Daily backup failed! Please check logs.' -SmtpServer 'smtp.example.com' -Priority High"
)
```
