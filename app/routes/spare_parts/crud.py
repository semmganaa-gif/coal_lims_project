...
def add_category():
    ...
    if request.method == 'POST':
        ...
        if not code or not name:
            flash('Код болон нэр заавал оруулна.', 'warning')
        elif SparePartCategory.query.filter_by(code=code).first():
            flash(f"'{code}' код бүртгэлтэй байна.", 'warning')
        else:
            ...
            db.session.add(cat)
            db.session.commit()
            flash(f"'{name}' категори нэмэгдлээ.", 'success')
            return redirect(url_for('spare_parts.category_list'))
...
def edit_category(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Категори шинэчлэгдлээ.', 'success')
        return redirect(url_for('spare_parts.category_list'))
...
def delete_category(id):
    ...
    if count > 0:
        flash(f"'{cat.name}' категорид {count} сэлбэг бүртгэлтэй. Эхлээд сэлбэгүүдийн категорийг өөрчилнө үү.", 'warning')
        return redirect(url_for('spare_parts.category_list'))

    name = cat.name
    db.session.delete(cat)
    db.session.commit()
    flash(f"'{name}' категори устгагдлаа.", 'warning')
    return redirect(url_for('spare_parts.category_list'))
...
def consume_spare_part(id):
    ...
    try:
        ...
        if quantity > spare_part.quantity:
            flash(f'Нөөц хүрэлцэхгүй! Байгаа: {spare_part.quantity} {spare_part.unit}', 'danger')
            return redirect(url_for('spare_parts.spare_part_detail', id=id))
