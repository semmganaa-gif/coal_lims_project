/* Fluorine (F,ad) — нэг мөрт (p1/p2) хувилбар. */
(function () {
  const ANALYSIS_CODE = 'F,ad';
  const TABLE_SEL = '#fluorine-analysis-table';
  const REPEATABILITY_LIMIT = getRepeatabilityLimit('F') ?? 0.05;
  ...
  function num(el){ if(!el) return null; const s=String(el.value??'').trim().replace(',', '.'); if(!s) return null; const n=Number(s); return Number.isFinite(n)?n:null; }
  ...
  function refs(sampleId){
    ...
    return {
      row,
      p1:{weight:row.querySelector(`#sample_${sampleId}_p1_weight`),result:row.querySelector(`#sample_${sampleId}_p1_result`)},
      p2:{weight:row.querySelector(`#sample_${sampleId}_p2_weight`),result:row.querySelector(`#sample_${sampleId}_p2_result`)},
  ...
  window.calculateAndDisplayForRow = function(sampleId, lockInputs=false){
    ...
    const p1w=num(r.p1.weight), p2w=num(r.p2.weight), p1r=num(r.p1.result), p2r=num(r.p2.result);
    let avg=null, diff=null, t_exceeded=false;

    const resetUI=()=>{ if(r.diff){r.diff.textContent='-'; r.diff.classList.remove('text-success','text-danger','fw-bold'); r.diff.title='';} if(r.avg) r.avg.textContent='-'; if(r.calc) r.calc.textContent='-'; };
    ...
    if(lockInputs && avg!=null){
      ...
      if(!isRejected){ [r.p1.weight,r.p1.result,r.p2.weight,r.p2.result].forEach(inp=>{ if(!inp)return; inp.readOnly=true; inp.classList.add('bg-light'); storage.set(K(inp.id+'_locked'),'true'); }); }
      return {
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    $table.querySelectorAll('tbody input.form-input').forEach(inp=>{
      const saved=storage.get(K(inp.id)); if(saved!=='') inp.value=saved;
      ...
      if(isRejected){ inp.readOnly=false; inp.classList.remove('bg-light'); storage.del(K(inp.id+'_locked')); }
      else if(wasLocked){ inp.readOnly=true; inp.classList.add('bg-light'); }
    });
    $table.querySelectorAll('tbody tr[data-sample-id]').forEach(tr=>{
      const sid=tr.dataset.sampleId; const anyInp=tr.querySelector('.form-input'); const isRejected=!!REJECTED_MAP[String(sid)];
      if(anyInp && storage.get(K(anyInp.id+'_locked'))==='true' && !isRejected){ window.calculateAndDisplayForRow(sid,false); }
    });
  });

  $table.addEventListener('input',(e)=>{
    const inp=e.target; if(!inp.classList.contains('form-input')||inp.readOnly) return;
    storage.set(K(inp.id), inp.value ?? '');
    const sid=inp.closest('tr[data-sample-id]')?.dataset.sampleId; if(!sid) return;
    const r=refs(sid); if(r){ if(r.diff){r.diff.textContent='-'; r.diff.classList.remove('text-success','text-danger','fw-bold'); r.diff.title=''; } if(r.avg) r.avg.textContent='-'; if(r.calc) r.calc.textContent='-'; }
  });

  (function keyboardNav(){
    const rows=Array.from($table.querySelectorAll('tbody tr[data-sample-id]'));
    function nextInRow(inputs, el, dir){ const idx=inputs.indexOf(el); return inputs[idx+dir] || (dir>0?inputs[0]:inputs[inputs.length-1]); }
    $table.addEventListener('keydown',(ev)=>{
      const el=ev.target; if(!el.classList?.contains('form-input')) return;
      if(!['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Enter'].includes(ev.key)) return;
      const row=el.closest('tr[data-sample-id]'); const inputsRow=Array.from(row.querySelectorAll('.form-input')); let target=null;
      ...
      if(ev.key==='ArrowUp'||ev.key==='ArrowDown'){
        const dir=ev.key==='ArrowDown'?+1:-1; const allRows=rows; const rowIdx=allRows.indexOf(row); const row2=allRows[rowIdx+dir];
        if(row2){ const idx=inputsRow.indexOf(el); const inputs2=Array.from(row2.querySelectorAll('.form-input')); if(inputs2.length) target=inputs2[Math.min(idx, inputs2.length-1)]; }
      }
