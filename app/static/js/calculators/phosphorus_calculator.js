/* Phosphorus (P,ad) — нэг мөрт (p1/p2) хувилбар.
   Хүхрийн калькуляторын яг ижил логик. REPEATABILITY_LIMIT-ийг хүсвэл тааруул.
*/
(function () {
  const ANALYSIS_CODE = 'P,ad';
  const TABLE_SEL = '#phosphorus-analysis-table';
  const REPEATABILITY_LIMIT = getRepeatabilityLimit('P') ?? 0.05;     // T хязгаар (abs) — хэрэгтэй бол өөрчил
  ...
  window.calculateAndDisplayForRow = function(sampleId, lockInputs=false){
    ...
    const p1w = num(r.p1.weight), p2w = num(r.p2.weight);
    const p1r = num(r.p1.result), p2r = num(r.p2.result);
    ...
    if (p1r!=null && p2r!=null){
      ...
      if (r.diff){
        ...
        if (t_exceeded){ r.diff.classList.add('text-danger','fw-bold'); r.diff.title = `T > ${REPEATABILITY_LIMIT} (exceeded)`; }
        else { r.diff.classList.add('text-success'); r.diff.title = `T ≤ ${REPEATABILITY_LIMIT} (OK)`; }
      }
    ...
    if (lockInputs && avg!=null){
      ...
      if (!isRejected){
        [r.p1.weight, r.p1.result, r.p2.weight, r.p2.result].forEach(inp=>{
          if(!inp) return; inp.readOnly = true; inp.classList.add('bg-light'); storage.set(K(inp.id+'_locked'),'true');
        });
      }
      return {
  ...
  $table.addEventListener('input', (e)=>{
    const inp = e.target;
    if (!inp.classList.contains('form-input') || inp.readOnly) return;
  ...
  (function keyboardNav(){
    ...
    function nextInRow(inputs, el, dir){
      ...
      return inputs[idx + dir] || (dir>0 ? inputs[0] : inputs[inputs.length-1]);
    }
    $table.addEventListener('keydown', (ev)=>{
      ...
      const row = el.closest('tr[data-sample-id]'); const sid = row?.dataset.sampleId;
      const inputsRow = Array.from(row.querySelectorAll('.form-input')); let target=null;
      ...
      if (ev.key==='ArrowUp' || ev.key==='ArrowDown'){
        ...
        const rowsArr = rows; const rowIdx = rowsArr.indexOf(row); const row2 = rowsArr[rowIdx+dir];
