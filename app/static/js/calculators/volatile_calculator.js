...
// --- ТОХИРГОО ---
const VOLATILE_REPEATABILITY_LIMIT = 0.5;
...
window.calculateAndDisplayForRow = function (sampleId, lockInputs = false) {
  ...
  if (lockInputs) {
    if (!isNaN(avg)) {
      ...
      console.log(`Sample ID ${sampleId} (Vad): оролтууд түгжигдлээ.`);
    } else {
      console.log(
        `Sample ID ${sampleId} (Vad): Түгжээгүй — дундаж NaN (Mad алга эсвэл дутуу).`
      );
      dataToSave = null;
    }
  }
...
// --- DOM READY ---
$(function () {
  console.log('Volatile Calculator Script Initializing...');
  ...
  console.log("Утгатай (Vad) дээжүүдэд 'X' товчийг нууж байна:", populatedSampleIds);
  ...
  console.log('Volatile Calculator Script Initialized.');
});
