...
- [ ] SSH нууц үгээр нэвтрэхийг хаасан уу? (доорх 14-р алхам)
- [ ] GitHub repo Private болгосон уу?

---

## 14-р алхам: Эх код хамгаалалт (ЧУХАЛ!)

Энэ хэсэг нь серверт байгаа кодыг хэн нэгэн хуулахаас хамгаална.

### Юу аль хэдийн хамгаалагдсан вэ?

1. **Лиценз систем** — Код нь серверийн hardware-д холбогдсон. Өөр сервер дээр ажиллахгүй.
2. **Docker .pyc хөрвүүлэлт** — Эх код (.py) нь Docker image-д байхгүй. Зөвхөн хөрвүүлсэн байтекод (.pyc) байна. Энэ нь хүний нүдээр уншигдахгүй.
3. **.dockerignore** — .env, .git, tests, docs зэрэг файлууд Docker image-д орохгүй.

### 14.1. GitHub Repository-г Private болгох

GitHub дээр код нээлттэй (Public) байвал **ДЭ-ЛХИЙ ДАЯАР** хүн бүр харна.

1. Браузераар https://github.com/semmganaa-gif/coal_lims_project руу орно
2. **Settings** (Тохиргоо) tab дарна
3. Хуудасны хамгийн доод хэсэгт **"Danger Zone"** байна
4. **"Change repository visibility"** дарна
5. **"Make private"** сонгоно
6. Нэрийг бичээд батална

Үүний дараа зөвхөн та болон таны нэмсэн хүмүүс л кодыг харна.

### 14.2. SSH түлхүүр ашиглах (нууц үгээр нэвтрэхийг хаах)

SSH түлхүүр бол "цахим нэвтрэх үнэмлэх". Нууц үг бичихийн оронд таны компьютерт хадгалагдсан файлаар нэвтэрнэ. Хэн нэгэн нууц үг таавал нэвтрэж чадахгүй.

**Windows компьютер дээрээ (PowerShell):**

```powershell
# 1. SSH түлхүүр үүсгэх (Enter, Enter дарна)
ssh-keygen -t ed25519 -C "coal-lims-admin"

# 2. Түлхүүрийг серверт хуулах
...
```bash
# 3. SSH тохиргоо — нууц үгээр нэвтрэхийг хаах
sudo nano /etc/ssh/sshd_config
```

Дараах 2 мөрийг олоод засна:
```
PasswordAuthentication no
PermitRootLogin no
```

Хадгалаад SSH дахин ачаална:
```bash
sudo systemctl restart sshd
```

**АНХААРУУЛГА:** Энэ хийхээсээ өмнө SSH түлхүүрээр нэвтэрч чадаж байгаа эсэхийг ЗААВАЛ шалгана! Шалгаагүй бол серверт нэвтрэх боломжгүй болно.

### 14.3. Firewall — зөвхөн шаардлагатай портуудыг нээх

```bash
# Бүх портыг хаах, дараа нь шаардлагатайг нь нээх
...
# HTTP ба HTTPS (вэб хуудас)
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp

# Firewall идэвхжүүлэх
sudo ufw enable
...
Зөвхөн 22, 80, 443 порт нээлттэй байх ёстой.

### 14.4. Серверийн хэрэглэгчийн эрх хязгаарлах

```bash
# LIMS-д зориулсан хязгаарлагдмал хэрэглэгч үүсгэх
sudo adduser lims-deploy

# Docker бүлэгт нэмэх (Docker ажиллуулах боломж)
sudo usermod -aG docker lims-deploy

# Код хавтасны эрх (зөвхөн энэ хэрэглэгч өөрчилж чадна)
sudo chown -R lims-deploy:lims-deploy /opt/coal_lims
sudo chmod -R 750 /opt/coal_lims
sudo chmod 600 /opt/coal_lims/.env
```

root хэрэглэгчээр SSH-ээр нэвтрэхийг хаасан (14.2 алхам) тул бусад хэрэглэгчид `/opt/coal_lims` хавтас руу нэвтрэх боломжгүй.

---

## Хамгаалалтын тойм

Бүх алхмыг хийсний дараа:

```
Хамгаалалтын давхаргууд:

Давхарга 1: GitHub Private Repo
   → Интернетээр код харах боломжгүй

Давхарга 2: SSH түлхүүр + Firewall
   → Зөвшөөрөлгүй хүн серверт нэвтрэх боломжгүй
...
Давхарга 5: Хэрэглэгчийн эрх + chmod
   → Container дотор root биш (lims user)
```
